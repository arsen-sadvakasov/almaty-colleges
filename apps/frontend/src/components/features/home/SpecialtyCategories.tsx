  ...
  return (
    <section className="py-24 bg-white dark:bg-neutral-900 transition-colors">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
        ...
        <p className="text-neutral-600 dark:text-neutral-400 max-w-2xl mx-auto mb-16 text-lg">
          {t('subtitle')}
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {CATEGORIES.map((category, idx) => (
            <Link 
              key={idx}
              href="/specialties"
              className="flex items-center gap-6 p-6 rounded-2xl border border-neutral-200 dark:border-neutral-800 hover:border-primary-500 dark:hover:border-primary-500 hover:shadow-md transition-all group text-left bg-neutral-50 dark:bg-neutral-800"
            >
              ...
              <div>
                <h3 className="font-medium text-neutral-900 dark:text-neutral-100 mb-1">{category.name}</h3>
                <p className="text-sm text-neutral-500 dark:text-neutral-400">{t('collegesCount', { count: category.count })}</p>
              </div>
            </Link>
          ))}
        </div>
